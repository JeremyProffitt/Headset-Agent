...
- (marker stays `[~]` until: re-ingestion deployed + `/chat` with `connection_type`/`brand` attrs returns a KB-grounded answer AND `aws bedrock-agent-runtime retrieve` with a metadata filter returns >0 results.)
