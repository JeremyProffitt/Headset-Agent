...
| A-03 | `[x]` | Author the microphone doc set (DM-1): not-working/too-quiet/muffled/mute-LED/levels-boost/permissions | new `knowledge-base/microphone/*.md` + sidecars | Fable | A-01,A-02 | Six docs in Likely-cause→Check→Steps→Verify format, Win10+Win11 paths, reviewed vs Tree 2 |
| A-04 | `[x]` | **Decide & provision the vector store** (see §5 Q-A). Spike S3 Vectors for Bedrock KB in us-east-1; else OpenSearch Serverless collection + policies; record decision + monthly cost | `infrastructure/template.yaml` KB section; decision in §5 | Opus | (Q-A) | Vector store deploys green; cost written down; teardown noted in `destroy.yml` |
| A-05 | `[x]` | `AWS::Bedrock::KnowledgeBase` (Titan Text Embeddings v2) + `DataSource` (KB bucket, ~300-token chunks/20% overlap) + scoped KB role | `infrastructure/template.yaml`; SSM `/kb-id`, `/kb-data-source-id` | Sonnet | A-04 | KB ACTIVE via pipeline; SSM params populated |
| A-06 | `[x]` | Ingestion automation: s3-sync + `start_ingestion_job` + poll to COMPLETE; wire into deploy.yml (replaces bare `s3 sync`) | new `scripts/sync-knowledge-base.py`; `deploy.yml` | Sonnet | A-05 | KB edits re-ingest automatically; job failure fails the step (no `continue-on-error`) |
...
| A-10 | `[x]` | Retrieval eval harness: golden questions (≥2/tree + 1/brand + mic) hit `Retrieve`, assert expected `tree_id` in top-3; real PR gate | new `tests/retrieval/`, `scripts/eval-retrieval.py`; `pr-validation.yml` | Sonnet | A-06 | ≥90% top-3 hit rate; gate fails PR below threshold |
...
| B-02 | `[x]` | Wire session store into the Lex handler (and `/chat` parity): load at top, merge to session attrs, persist after response (incl. escalation/Close) | `cmd/lex-lambda/main.go` | Sonnet | B-01 | 5-turn call: state survives in DynamoDB; chat shares the store; graceful degrade + log on read failure |
...
| ID | St | Task | Files / resources | Model | Deps | DoD |
|---|---|---|---|---|---|---|
| WS-C-01 | `[x]` | `AWS::Connect::HoursOfOperation` (M–F 09:00–17:00 America/New_York, default) | `template.yaml` | Haiku | — | Deploys via GHA; ARN → output + SSM `/connect/hours-arn` |
| WS-C-02 | `[x]` | Escalation `AWS::Connect::Queue` bound to hours, MaxContacts set | `template.yaml` | Haiku | C-01 | Queue ACTIVE; ARN → SSM `/connect/escalation-queue-arn` |
| WS-C-03 | `[x]` | `AWS::Connect::RoutingProfile` (escalation + skill queues, VOICE concurrency 1) | `template.yaml` | Sonnet | C-02 | Deploys; default outbound queue set |
| WS-C-04 | `[x]` | ≥1 `AWS::Connect::User` — password from GH Actions secret `CONNECT_AGENT_PASSWORD` via `NoEcho` param; security-profile ARN looked up in deploy.yml; **no Secrets Manager** | `template.yaml`, `deploy.yml` | Sonnet | C-03 | User logs into CCP; password never in logs/template |
...
| WS-C-06 | `[x]` | **Fix the dead-end transfer (OPS-1)**: `UpdateContactTargetQueue` → `CheckHoursOfOperation` → `TransferContactToQueue` with error branches to C-09/10 | `contact-flow-lex.json`, `template.yaml` | Opus | C-02,C-05 | Live: "agent" rings the C-04 user's CCP; no silent disconnect |
...
| WS-D-10 | `[x]` | SEC-7 no-payment guardrail (overlaps A-09): denied-topic payments/card + card-pattern PII filter + system-prompt clause + refusal test | `template.yaml`/`create-agents.py`, `internal/agents/bedrock.go`, tests | Sonnet | — | "Take my card number" → refusal; test green |
...
| ID | St | Task | Files / resources | Model | Deps | DoD |
|---|---|---|---|---|---|---|
| E-1.0 | `[x]` | **Test chat front-end (password-gated), user-requested.** Static `website/index.html`: client-side password gate (`testing-me`) → simple chat UI POSTing to `ChatApiUrl`. Pipeline `publish-website` job injects the real Chat API URL + syncs to the website bucket. Password documented in README.md. **Soft test gate only** (password ships in page; real auth = SEC-5/WS-D-07/08). | `website/index.html`, `.github/workflows/deploy.yml` (publish-website job), `README.md` | Sonnet | (ChatApi exists) | Page loads, gate accepts `testing-me`, chat round-trips to /chat. Precursor to the full WCAG UI E-1.3 |
...
| WS-F-04 | `[x]` | **Config-drift canary** (hourly EventBridge micro-Lambda): metrics `AgentIdIsPlaceholder`/`PhoneNumberPending` from SSM + alarms (~$0.60/mo) — turns today's silent PLACEHOLDER warning into a page | new `cmd/canary-lambda/`, `template.yaml` | Fable | F-01 | Manually setting a param to PLACEHOLDER alarms within 2h |
...
| WS-G-09a | `[ ]` | **Install the Nova Sonic safety gate (REL-4; M0).** Default `DeployNovaSonicPath=false` (or make the Nova contact flow transfer to the Lex flow when the path is disabled) so the still-stubbed Path B can NEVER serve a live caller. **Split from WS-G-09 per the §5.1 review: the old single task was an M0 gate dep'd on M3.5's WS-H-06 — impossible, and `deploy_nova_sonic` currently defaults `true`, leaving the stub reachable.** | `template.yaml`, `deploy.yml`, `contact-flow-nova-sonic.json` | Fable | C-05 | Calling the Nova number (or invoking the Nova path) reaches Path A / a safe message — never the stub |
| WS-G-09b | `[ ]` | **Flip the gate to live (M3.5).** Set `DeployNovaSonicPath` default `"true"` only when WS-H-06 acceptance passes. | `template.yaml`, `deploy.yml` | Sonnet | WS-H-06, WS-G-09a | Flipping the flag after WS-H acceptance serves real Nova Sonic audio; rollback-to-Path-A verified |
...
- A-02 [x] (38 KB metadata sidecars) landed in the 37314b7 push.
  4. Attempts 3–5 still rolled back. The FAILED-filtered diagnostics (after fixing the diagnostics step to query `ResourceStatus=*FAILED*` server-side + JSON — the newest-40 window had been all rollback noise) finally named the **true root cause**: the canary's `ReservedConcurrentExecutions: 1` — *"decreases account's UnreservedConcurrentExecution below its minimum value of [10]"*. This account's Lambda concurrency limit is too low to reserve any concurrency. Fix (e5ba655): removed it; **WS-F-08 (reserved-concurrency caps) is BLOCKED on a Service Quotas increase** (documented inline in template + here). The Connect flow was a red herring — the flow had been fine; the canary failed first and cascade-cancelled the flow update.
  5. **Run 27415620398 (e5ba655) — GREEN.** Connect Queue/Hours/RoutingProfile/User + consent + canary + B-02/B-06 handler all deployed. Then re-added the escalation transfer (dfce6b5, `Compare` on `$.Lex.SessionAttributes.escalation_requested` → UpdateContactTargetQueue → TransferContactToQueue) — **run 27416280442 GREEN**.
- **Net M0 status:** WS-C-01/02/03/04/05 [x], WS-C-06 [x] (structurally deployed; runtime "agent→queue" routing is the phone test), WS-D-01 [x] (Lex consent + policy doc; recording STORAGE D-01b/01c still pending), WS-F-04 [x]. Lessons baked in: cfn-lint gate + FAILED-filtered CFN diagnostics now catch property/quota errors fast.

### M1 Working Log

---
**[2026-06-12] Claude(Opus orchestrator + Fable subagent) — A-03 microphone doc set (DM-1) — [ ]→[~]**
- Authored 6 RAG-only mic docs under `knowledge-base/microphone/` (the #1 caller complaint): `mic-not-working`, `mic-too-quiet`, `mic-muffled`, `mic-mute-led`, `mic-levels-boost`, `mic-permissions` — each `Likely Cause → Check → Steps → Verify`, ~700–800 tokens, with distinct Windows 10 vs Windows 11 menu paths, grounded strictly in Tree 2 / Tree 5 / Tree 7 / `windows/win-2.*` / `windows-sound-settings.md` (no invented menu paths). Subagent flagged conservative omissions where the source KB had no coverage (named "Microphone Boost" slider, mic foam/windscreen, per-brand mute-LED colors, codec tuning) — kept those conditional/omitted rather than fabricating.
- Sidecars use the **correct `<name>.md.metadata.json` convention** (the A-02 bug fixed earlier this session): all 6 are `*.md.metadata.json`, single-line `{"metadataAttributes":{...}}`, frozen vocab `symptom=mic_not_working, brand=any, platform=windows, tree_id=tree-2, connection_type∈{usb,any}`. These are RAG-only (NOT a new tree) per §5.1 #7 — `tree_id=tree-2` so they retrieve within the Tree-2 mic flow.
- Orchestrator-verified locally: 6 docs + 6 sidecars, zero misnamed, all JSON valid, all sections + Win10/11 present, vocab correct.
- **VERIFIED → [~]→[x] (pipeline run 27443850490, GREEN):** Sync Knowledge Base re-ingested the 6 mic docs; a `retrieve` probe for "my microphone is too quiet and people say I sound faint" returned `s3://…/microphone/mic-too-quiet.md` in top-2 (top-3 also had tree-6). The **A-10 Retrieval Eval Gate stayed green (21/21)** with the new tree-2-tagged docs present — confirming they don't displace the golden tree docs. Mic content is now live + retrievable.
...
---
**[2026-06-12] Claude(Opus orchestrator) — B-02/B-06 [x] (deployed), B-04 [x], B-05 [x]**
- B-02/B-06 (Sonnet): session store wired into lex + /chat; DetectEscalation exposes FrustrationDelta; handler accumulates frustration_count across turns (honest-truth #4 fixed); 3-turn escalation test green. **Live (run 27415620398).**
- B-04 (Fable): `internal/triage` engine + trees (pre-flight + Trees 1–8) + classifier, pure (no Bedrock in core); `Step.Branches` for n-way forks; 96.3% cover, 44 path-walk tests; all 10 EscalationReasons reached; 8 index utterances classify keyword-only.
- B-05 (Fable): triage engine wired into handleLexRequest + verify loop (CC-3). New `internal/handlers/prompts.go` (read-aloud catalog ~95 keys from troubleshooting.md §1) + `triage_turn.go` (ParseOutcome, per-turn driver, Load/SaveTriageState via B-02 accessors). Engine navigates; Bedrock only renders free-form follow-ups grounded in the step KBDoc (degrades gracefully). Escalation/RMA terminals → existing BuildEscalationResponse + OPS-2 attrs (triage_tree/triage_step/attempted_steps); resolved→disposition+Close. **Fixed a latent B-06 mirror bug** (mutated counters now mirrored into response Lex attrs). handlers 79.1%, internal 87.6%, all green. /chat triage deferred. Deployed in the B-05 push.
- A-04/A-05/A-06/A-11 (Opus): RAG foundation **deployed GREEN (run 27418839958, first try; ingestion succeeded — 38 docs in the vector index).** **Q-A resolved in practice: S3 Vectors is GA + fully CFN-native in us-east-1 — no Pinecone fallback needed.** Added `KbVectorBucket`/`KbVectorIndex` (1024-dim cosine, AMAZON_BEDROCK_TEXT non-filterable, sidecar keys filterable), least-priv `KnowledgeBaseServiceRole`, `HeadsetKnowledgeBase` (Titan Text Embeddings v2) + `HeadsetKbDataSource` (300-tok/20% chunks), SSM `/kb-id` `/kb-data-source-id` `/kb-arn`. `scripts/sync-knowledge-base.py` (s3 sync + start_ingestion_job + poll-to-COMPLETE, fail-closed) replaced the bare `s3 sync` in deploy.yml. Lex Lambda got `KB_ID`/`KB_ARN` env + scoped `bedrock:Retrieve` on the KB ARN (A-11). Cost: pennies/mo (consumption-based; OpenSearch ~$175/mo avoided). **Gotcha:** AWS::S3Vectors::{VectorBucket,Index} do NOT support `Tags` (cfn-lint E3002 caught it locally — the subagent's older venv cfn-lint missed it; my cfn-lint 1.42 caught it). **Next: A-07 (collapse 4 orphan agents→1 + associate_agent_knowledge_base) + A-08 (Lambda calls bedrock:Retrieve with metadata filters + grounding prompt) to make retrieval actually flow into answers.**

### M2 Working Log

---
**[2026-06-12] Claude(Opus orchestrator) — B-07 Lex slots → metadata-filtered retrieval — [ ]→[~]**
- Decision (architecture): the live bot drives dialog via an in-Lambda ElicitIntent conversational loop — it never `Delegate`s to Lex, so Lex's native required-slot elicitation never fires. Making slots `Required` would be inert (or, via a Delegate rearchitecture, would risk breaking the working voice/chat flow — Risk R5/R1). **Chosen approach:** slots declared `Optional` with `TopResolution` (Lex NLU resolves them + synonyms from any utterance — satisfies "fills 3 slots in one shot"); the Lambda READS filled slots each turn, persists them, threads them into the deterministic classifier, and feeds KB metadata filters. **Net change is purely additive — zero behavior change to the existing flow when no slots are present (all prior handler tests pass unchanged).**
- Decision (elicitation — revised mid-build): I first added an active one-time `connection_type` clarifier turn, but it inserts a question into EVERY troubleshooting call (a permanent UX regression on a live bot) and rippled through the multi-turn handler tests. Reverted it. **Final policy:** `issue_type` = symptom; its single active elicitation is the existing `symptomClarifyPrompt` (already ≤once) — this is the DoD's "missing slots elicited ≤once". `connection_type`/`headset_brand` are captured opportunistically from NLU (never actively asked — they degrade gracefully to "no filter" when unknown) and are configured at the Lex slot level with elicitation prompts + `MaxRetries:1` + telephony-default DTMF (the genuine, deployed home for the "elicit/DTMF-spelling fallback" affordance, ready for a future B-10/B-13 ladder).
- Decision (filter semantics — important): most KB docs are tagged `brand:"any"` (33/38) and `connection_type:"any"|"usb"` — an *equals* filter on brand=jabra would EXCLUDE every generic doc and starve retrieval. So the filter uses **IN-with-"any"** semantics: `connection_type ∈ {requested,"any"}`, `brand ∈ {requested,"any"}`, AND-combined. Extended `RetrieveAndGenerateRequest` with `FilterAnyOf map[string][]string` (builds `RetrievalFilterMemberIn`); kept the existing exact `Filters map[string]string` for back-compat. `tree_id` is deliberately NOT filtered (our internal navigation; over-filtering risks empty retrieval on legitimate cross-tree questions — connection_type/brand are the stable caller facts). The Lex slot is named `headset_brand`; the handler normalizes it onto the KB sidecar metadata key `brand`.
- Vocabulary is the frozen A-02 sidecar set (§5.1 #6): connection_type{usb,bluetooth,dect,wireless_dongle}, brand{jabra,poly,logitech,epos,yealink}, issue_type→8 symptom classes. Slot-type synonyms added (plantronics→poly, sennheiser→epos, dongle/adapter→wireless_dongle, etc.). Out-of-vocab slot values are dropped before they reach the filter.
- Files touched:
  - `infrastructure/template.yaml` — 3 custom `SlotTypes` (ConnectionType/HeadsetBrand/IssueType, TopResolution + synonyms) + 3 Optional slots + `SlotPriorities` on TroubleshootIntent + 3 slot-bearing sample utterances. (cfn-lint gotcha: `ResolutionStrategy` enum is UPPER_SNAKE `TOP_RESOLUTION`, not `TopResolution` — E3030; caught locally, fixed. Final cfn-lint E-level = 0.)
  - `internal/agents/bedrock.go` — `RetrieveAndGenerateRequest.FilterAnyOf`; `buildRetrievalFilter(exact, anyOf)` now emits equals + `In` conditions (AND-combined, deterministic ordering).
  - `cmd/lex-lambda/main.go` — `resolvedSlots()`/`slotValue()` (reads Lex V2 slot shape, intent→interpretations fallback, vocab-guards connection/brand), `mergeSlots()` (persist + sticky effective view), `kbFilters()` (IN-with-any); wired `effectiveSlots` into `TriageDeps.Slots` and `kbFilters(sess)` into all three RAG callsites (free-form, generic lex turn, /chat).
  - `internal/handlers/triage_turn.go` — `TriageDeps.Slots`; threaded into all 3 `Classifier.Classify` calls (opening turn, awaiting-symptom clarifier, ErrSymptomRequired path).
  - `internal/session/store.go` — `KeyConnectionType`/`KeyBrand`/`KeyIssueType` consts.
  - Tests: `bedrock_test.go` (+3 In/any-of filter cases), `main_test.go` (+7: slot parse/normalize/vocab-drop/interpretations-fallback, merge stickiness, kbFilters, issue_type→symptom resolution, slots→filters end-to-end).
- Commands: `go build/vet ./...` clean; `go test ./...` PASS; internal coverage agents 91.1% / handlers 79.1% / session 88.2% (gate 60%); gofmt clean (CRLF-aware: `tr -d '\r' | gofmt -l`); `cfn-lint` E-level = 0.
- Gotchas for the next agent: (1) brand/connection filters MUST stay IN-with-"any" — never switch to exact equals or retrieval starves on the 33 generic docs. (2) The Lex slots are Optional by design (the ElicitIntent loop owns dialog); do NOT make them Required expecting auto-elicitation — it won't fire without a `Delegate` rearchitecture. (3) `headset_brand` slot ↔ `brand` metadata key mismatch is intentional and handled in `resolvedSlots`.
- **Pipeline run 27441027471 (commit 8ccd301) — GREEN** (all 12 jobs incl. SAM deploy with the Lex bot rebuild for the new slot types, KB re-sync, agent creation, integration tests).
- **CRITICAL latent-bug found during `/chat` verification (was A-02, surfaced by B-07):** the Bedrock filter returned ZERO docs for ANY metadata filter (`equals connection_type=usb` → 0; `in [any,usb]` → 0). Direct `aws bedrock-agent-runtime retrieve` proved the retrieved chunks carry ONLY `x-amz-bedrock-kb-*` system metadata — **the A-02 sidecar fields (symptom/connection_type/brand/platform/tree_id) were never ingested**. Root cause: the 38 sidecars were named `<name>.metadata.json`, but Bedrock requires the sidecar filename = **source filename INCLUDING extension** + `.metadata.json`, i.e. `<name>.md.metadata.json`. Wrong name → Bedrock silently ignored every sidecar. A-02's "retrieval filtering" DoD was never actually exercised until B-07 used the filters, so this stayed hidden. **Fix:** `git mv`'d all 38 `*.metadata.json` → `*.md.metadata.json` (content unchanged; already correct `{"metadataAttributes":{...}}`). The sync script (`scripts/sync-knowledge-base.py`) uploads the new names, deletes the stale old ones, and re-ingests; Bedrock re-processes a doc when its sidecar changes, so metadata becomes filterable. A-02 stays `[x]` (metadata content was always correct — only the filename convention was wrong); fix logged here.
- **VERIFIED → [~]→[x] (pipeline run 27441572727, commit 8d3912c, GREEN incl. Sync Knowledge Base re-ingest):**
  - `aws bedrock-agent-runtime retrieve` with `in connection_type=[any,usb]` → **5 results** (was 0 before the sidecar fix); `in brand=[any,jabra]` → **5 results**. Retrieved chunk metadata now carries `{symptom, connection_type, brand, platform, tree_id}` (was only `x-amz-bedrock-kb-*`).
  - `/chat` `aws lambda invoke --profile paul --function-name headset-lex-orchestrator-prod` with body `{connection_type:usb, brand:jabra}` + "My Jabra USB headset has no sound" → real KB-grounded persona answer naming the **Jabra** headset and USB-port + Sound-Settings steps (was the Bedrock zero-results refusal "Sorry, I am unable to assist…" before the fix). End-to-end chain proven live: slot/attrs → `kbFilters` IN-with-"any" → filtered `RetrieveAndGenerate` → grounded answer.
  - Handler-side classification proven by unit test (`TestHandleLexRequest_IssueTypeSlotResolvesSymptom`): a filled `issue_type` slot resolves the symptom deterministically even with no keyword match.
  - **One DoD clause is voice-only (Q-I): "fills 3 slots in one shot" via Lex ASR/NLU** needs a live phone call (no audio harness; `/chat` is direct RAG and doesn't run Lex NLU). The slot types (TopResolution + synonyms) + slots + sample utterances are DEPLOYED (SAM run 27441027471 rebuilt the bot) and the Lambda-side slot read/normalize/merge is unit-tested — so the NLU one-shot fill is structurally complete, pending the Q-I live-call batch like other voice DoDs. The CORE deliverable (values → retrieval filters) is fully verified live, so B-07 is marked `[x]`.

### M3 Working Log
_(empty)_

---
**[2026-06-12] Claude(Opus orchestrator + Sonnet subagent) — A-09 Bedrock Guardrails — [ ]→[~]**
- Decision (architecture, critical): the guardrail is attached to the **Lambda-side `RetrieveAndGenerate` `GenerationConfiguration`** (the effective live answer path per §5.1 #5) AND to the agent via `create-agents.py` (belt-and-suspenders). Attaching only to the agent would be dead config — the live `/chat`/voice path never calls `InvokeAgent`. Contextual grounding (GROUNDING 0.7 / RELEVANCE 0.5) works on the RAG path because Bedrock passes the retrieved chunks to the guardrail as the grounding source automatically.
- Guardrail policy (`AWS::Bedrock::Guardrail` `HeadsetGuardrail` + `HeadsetGuardrailVersion`): DENY topics Payments (SEC-7) / LegalAdvice / MedicalAdvice (each with definition + examples); content filters HATE/INSULTS/SEXUAL/VIOLENCE/MISCONDUCT at MEDIUM in+out, PROMPT_ATTACK HIGH-in/NONE-out (NONE required for prompt-attack output); PROFANITY managed word list; PII BLOCK on card number/CVV/expiry + US bank acct + PIN, ANONYMIZE on phone/email/address; ContextualGrounding GROUNDING≥0.7 / RELEVANCE≥0.5. Safe blocked-input/output messaging.
- Wiring: SSM `/headset-agent/prod/guardrail-id` + `/guardrail-version`; Lambda env `GUARDRAIL_ID`/`GUARDRAIL_VERSION` (`!GetAtt`); IAM least-priv `bedrock:ApplyGuardrail` scoped to `HeadsetGuardrail.GuardrailArn`. `bedrock.go` `RetrieveAndGenerateRequest` gained `GuardrailID`/`GuardrailVersion` (sets `GuardrailConfiguration` only when BOTH non-empty — graceful no-op otherwise); `main.go` `guardrailConfig()` reads env and sets it on all 3 RAG callsites. `create-agents.py` reads the SSM params and attaches `guardrailConfiguration` to create/update_agent, **retrying without it on failure** (a guardrail quirk can't break agent provisioning).
- Overlap: this is the Bedrock denied-topic layer WS-D-10 was waiting on (WS-D-10's in-Lambda payment detector already shipped); once A-09 verifies live, WS-D-10 → [x].
- Files: `infrastructure/template.yaml`, `internal/agents/bedrock.go` (+3 guardrail tests in `bedrock_test.go`), `cmd/lex-lambda/main.go` (+1 test in `main_test.go`), `scripts/create-agents.py`.
- Local gates (orchestrator-verified, not just subagent-reported): `go build/vet ./...` clean; `go test ./...` PASS; internal cover agents 91.3% / handlers 79.1% / session 88.2% (gate 60%); gofmt CRLF-aware clean; **cfn-lint E-level = 0**; `py_compile` ok.
- **VERIFIED → [~]→[x] (pipeline run 27442386420, commit 3492758, GREEN incl. SAM deploy of the new Guardrail resource + Create Bedrock Agents attach). Guardrail `kt5712nzhrcz` v1.** Live probes (`--profile paul`):
  1. `apply-guardrail` legal-advice input → `GUARDRAIL_INTERVENED`, topic **LegalAdvice** (proves denied topics; bypasses the in-Lambda payment detector so it's the guardrail itself).
  2. `apply-guardrail` "charge my Visa 4111…" → `GUARDRAIL_INTERVENED`, PII **CREDIT_DEBIT_CARD_NUMBER/EXPIRY** + topic **Payments** (payment probe blocked).
  3. `apply-guardrail` contextual-grounding test (planted ungrounded output) → `GUARDRAIL_INTERVENED`, **GROUNDING score 0.0** (<0.7), **RELEVANCE 0.37** (<0.5) — planted off-KB answer rejected.
  4. **Live `retrieve-and-generate`** (KB `FK7UBC8GH6` + model `us.anthropic.claude-sonnet-4-6` + the guardrail, exactly as the Lambda constructs it): legal query → `guardrailAction=INTERVENED` + blocked message; in-scope "USB headset no sound" → `guardrailAction=NONE` + full grounded answer (no over-blocking).
...
- **VERIFIED → [~]→[x] (pipeline run 27443311831, GREEN):** the `Retrieval Eval Gate` job ran in CI after `sync-knowledge-base` and logged `Retrieval eval: 21/21 passed (100.0%) threshold 90.0%` → `GATE PASSED`. It is now a live blocking gate on every deploy (a retrieval-quality regression — or a re-break of the A-02 metadata fix — fails the deploy).
...
**Q-A…Q-H are resolved.** BUT a 2026-06-12 Fable-team plan review (Meridian/Vox/Bedrock/Echo) surfaced NEW outstanding questions + autonomous-build defects — see §5.1. The plan is **not** fully autonomously-closeable until the §5.1 human-gated items (Q-I…Q-P) are answered.

### 5.1 Fable-team plan review (2026-06-12) — outstanding questions & autonomous-build fixes

**Verdict (consensus): ~80% of remaining work is autonomously buildable; M0/M2/M3.5 exit gates cannot CLOSE without human action.** New questions for the user (Q-I…Q-P) + plan defects (baked-fix vs needs-answer):

| # | Outstanding question (needs a human decision) | Blocks | Status |
|---|---|---|---|
| Q-I | **Live-call verification protocol.** ~15 DoDs need a human to place/hear a real phone call (C-06 runtime, C-08/09/10, D-01b/01c, B-08/09/11, F-03, all WS-H). No automated outbound-call/audio harness exists. Batch human phone-test checkpoints, OR fund a SIP/Twilio caller-bot harness, OR accept structural-only verification for voice? | M0/M1/M2/M3.5 voice DoDs | ⚠ OPEN |
| Q-J | **OIDC cutover + static-key deactivation (WS-D-05).** Destructive; needs a human "go" after one green deploy + green destroy dry-run on the OIDC role, plus a break-glass key-reactivation procedure. destroy.yml (8 static-key uses) must be in scope. Proceed now, or stay on static keys for v1? | WS-F-10, E-9.6 crons; SEC-3 | ⚠ OPEN |
| Q-K | **WS-F-08 reserved concurrency** is blocked by this account's Lambda unreserved floor (min 10). File a Service Quotas increase (external AWS approval, async), or formally DESCOPE WS-F-08 and rely on WS-F-07 budgets + alarms? | WS-F-08 | ⚠ OPEN |
| Q-L | **SES production access + sender identity** for email-the-steps (E-1.4) and voicemail-email (C-09). Sandbox-exit is a human AWS support case + domain/sender verification. From-address/domain? | E-1.4, C-09 | ⚠ OPEN |
| Q-M | **SMS origination + verification (E-1.2/E-7.3/E-1.5).** A Connect *voice* toll-free number is NOT SMS-capable; AWS End User Messaging needs TF-SMS verification or 10DLC registration (human form, days–weeks). §7 also says "toll-free avoided" — contradiction. Which origination path, and start verification now? (Long pole for the AX-1 Must.) Also support "text a different number" for relay/videophone callers whose ANI can't receive SMS. | E-1.2, E-7.3, AX-1 | ⚠ OPEN |
| Q-N | **Nova Sonic return-audio bridge (WS-H-01).** Connect KVS media streaming is INGEST-ONLY; injecting synthesized audio back into a live call needs Chime SDK Voice Connector / SIP Media Application (not Fargate+KVS alone). §7 cost assumes Fargate-only. Accept the Chime SDK SMA architecture + its cost, or keep Path B fenced? | all WS-H | ⚠ OPEN |
| Q-O | **/chat auth vs public chat page (D-08 vs E-1.0/E-1.3).** A shared `X-Api-Key` embedded in a public web-chat page isn't real auth. Accept token-as-throttle (CloudFront-injected origin header + rate limits), or require stronger auth? | WS-D-08, E-1.3 | ⚠ OPEN |
| Q-P | **Human-review sign-offs:** E-8.1 disposition taxonomy ("Product-reviewed"), E-9.3 30-conversation human-labeled judge-calibration set + 10% weekly audit, WS-D-11 accepted-risk register signer. May an agent self-approve PROVISIONAL versions (per the §5 autonomy rule), or do these wait for Jeremy? | E-8.2→C-12→C-13→D-09; E-9.3; D-11 | ⚠ OPEN |

**Autonomous-build PLAN DEFECTS — fixes baked in this review (no human needed):**
1. **WS-G-09 split (critical):** it's an M0 gate but dep'd on WS-H-06 (M3.5) → impossible under §0's pickup rule, and meanwhile `deploy_nova_sonic` defaults `true` so the Path-B STUB is reachable. → **WS-G-09a** (install gate, default `DeployNovaSonicPath=false` / Nova flow transfers to Lex; M0; deps C-05) + **WS-G-09b** (flip default to `true`; M3.5; deps WS-H-06). [see task-table edits]
2. **§2 milestone gating is "soft":** M1 work (B-01..06, A-04/05/06/11) was correctly done while M0 is open. Rule amended: a milestone's tasks may start once their own deps are `[x]`, regardless of prior-milestone exit-gate, EXCEPT where a task's DoD explicitly requires a prior gate.
3. **Stale file refs:** post WS-C-05 the **inline `LexContactFlow` in template.yaml is the single source**; tasks C-07/C-09/C-13/E-1.5/E-2.3/WS-D-01c/B-09 that name `contact-flow-lex.json`/`contact-flow-*.json` must edit the inline flow instead.
4. **Pace-control single owner:** B-12 and E-2.1/E-2.2 are the same feature (VX-5/AX-2) in two workstreams editing `lex.go`. **Owner = WS-E (E-2.x); B-12 is folded into E-2.2** (delete B-12 as separate work).
5. **Retrieval ownership decision (A-07/A-08):** **Lambda-side `bedrock:Retrieve`** is the chosen architecture (A-11 already granted the IAM): A-08 = the Lex handler calls `Retrieve` with metadata filters (tree_id/brand/connection_type) and injects chunks into the prompt; A-07 still collapses to one agent but the KB is queried Lambda-side, not only via agent association. A-09 grounding-check attaches to that retrieve-then-generate flow.
6. **Controlled vocabulary is canonical + frozen:** the A-02 sidecar values ARE the schema (symptom{9}, connection_type{any,usb,bluetooth,dect,wireless_dongle}, brand{any,jabra,poly,logitech,epos,yealink}, platform{any,windows,genesys,teams,zoom,webex}, tree_id{preflight,tree-1..8,escalation,none}). A-03/DM-2..5/B-07 consume it; adding a value requires updating the sidecars + B-07 slot types together.
7. **DM-2..5 are RAG-only domains** (no new triage trees): B-05's handler answers them via Lambda Retrieve outside the 8-tree engine (engine stays closed). 
8. **B-13's `asr_low_confidence`** is absorbed into the E-8.1 disposition/escalation enum (no forked reason).
9. **Pull-forward (cheap, no deps, prod-safety):** WS-F-03 (Connect MissedCalls/queue alarms) and WS-F-07 (Budgets) move ahead of further WS-C flow edits; recording/Contact Lens already live so budgets matter now.
10. **Flow-edit safety convention:** every WS-C flow change must (a) be a single atomic template change, (b) be followed by a test call, (c) revert = `git revert` + push (~5 min exposure) — to be written into WS-G-11's runbook. WS-G-08 + removing setup-connect's `continue-on-error` ship in the same PR.
11. **Marker hygiene:** WS-C-06 is structurally deployed but its live agent→queue routing is pending Q-I; WS-F-01 SNS subscription now confirmed by the user.
