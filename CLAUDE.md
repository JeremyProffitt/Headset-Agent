...
### Local AWS CLI
The GitHub Actions pipeline deploys to account `759775734231` (the default profile). For local AWS CLI calls, use the default profile/credentials — they target the deploy account.

- Read-only diagnostics against the deploy account are fine and encouraged (e.g. `aws connect ... --region us-east-1`, CloudWatch logs, `describe-stack-events`, Bedrock model checks).
...
- This does NOT change the deploy policy below: infrastructure changes still go through the GitHub Actions pipeline, never `sam deploy`/`aws cloudformation deploy` from local. Local CLI is for **inspection/diagnostics** and explicitly-authorized operational one-offs (e.g. the user asked you to claim/release a phone number).
